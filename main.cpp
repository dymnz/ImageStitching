...
int mainStaticStitching(int imageCount, char *imageStr[]){
	...
	for (int i = 0; i < vImage.size(); i++)
		vBaseImage.push_back(new BaseImage(vImage[i]));
	...
	for(int i = 0;i < routeIndex.size();i++)
	{
		...
		{
			if((matchTracker.getPairHomography(route[j], route[j+1])).at<double>(0,0) != 0)
			{
	...
	for(int i = 0;i < imageCount;i++)
	{
		...
		{
			if((matchTracker.getPairHomography(i, j)).at<double>(0,0) != 0)
				printf("1 ");
			else
				printf("0 ");
		}
